...
func createLLMClient() (engine.Classifier, error) {
	...
	case "claudecode":
		...
		// Set default model if not specified - just use "opus"
		if config.Model == "" {
			config.Model = "opus"
		}
...
// createRawLLMClient creates a raw LLM client for analysis operations.
// This returns the raw llm.Client interface instead of engine.Classifier.
func createRawLLMClient() (llm.Client, error) {
	// Read LLM configuration from viper
	provider := viper.GetString("llm.provider")
	if provider == "" {
		provider = "openai" // default provider
	...
	// Override max tokens for analysis (needs more for complex responses)
	...
	// Create the raw client using the factory function
	return llm.NewClient(config)
}

// createAnalysisLLMClient creates an LLM client specifically for analysis tasks.
// It forces the use of opus model for Claude Code for best analysis results.
func createAnalysisLLMClient() (llm.Client, error) {
	// Read LLM configuration from viper
	provider := viper.GetString("llm.provider")
	if provider == "" {
		provider = "openai" // default provider
